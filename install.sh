...
sudo cpanm --force URI::Encode Digest::SHA DBD::Pg

# -----------------------------
# Создание базы и пользователя PostgreSQL
# -----------------------------
echo "[INFO] Creating PostgreSQL database and user..."
echo "[INFO] Creating PostgreSQL database and user..."

if ! sudo -su postgres psql -tAc "SELECT 1 FROM pg_database WHERE datname='${POSTGRES_DB}'" | grep -q 1; then
    sudo -su postgres psql -c "CREATE DATABASE ${POSTGRES_DB};"
    echo "[INFO] Database ${POSTGRES_DB} created."
else
    echo "[INFO] Database ${POSTGRES_DB} already exists."
fi

if ! sudo -su postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='${POSTGRES_USER}'" | grep -q 1; then
    sudo -su postgres psql -c "CREATE USER ${POSTGRES_USER} WITH PASSWORD '${POSTGRES_PASSWORD}';"
    echo "[INFO] User ${POSTGRES_USER} created."
else
    echo "[INFO] User ${POSTGRES_USER} already exists."
fi

sudo -su postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE ${POSTGRES_DB} TO ${POSTGRES_USER};"

sudo -su postgres psql -d "${POSTGRES_DB}" <<EOF
...
python3 -m pip install -r "https://raw.githubusercontent.com/privacyidea/privacyidea/v${PI_VERSION}/requirements.txt" psycopg2-binary
...
sudo tee /etc/freeradius/3.0/clients.conf > /dev/null <<EOSU
client 10.210.1.0/0 {
